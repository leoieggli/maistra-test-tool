  ...
  echo "Need to inform obrigatory fields!"
    ...
    try {
        ...
        stage("Create New Project"){
            ...
            wrap([$class: 'MaskPasswordsBuildWrapper', varPasswordPairs: [[var: 'IKE_PWD', password: KEY]], varMaskRegexes: []]) {
                sh """
                    oc login -u ${IKE_USER} -p ${IKE_PWD} --server=${params.OCP_SERVER} --insecure-skip-tls-verify=true
        ...
        stage("Notify Results"){
            catchError(buildResult: 'SUCCESS', stageResult: 'FAILURE') {
                ...
                def executedBy = util.whoBuild(util.getWhoBuild())
                ...
                emailext body: "<b>Started by:</b> ${executedBy}<p><b>Message:</b> currentBuild.result", subject: "Build number ${env.BUILD_NUMBER} - currentBuild.result", to: "lsouza@redhat.com"
            }
        }
    }
}